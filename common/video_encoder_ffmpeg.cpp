...
//            fprintf(stderr,"got pic flags:%d,key frame:%d\n",_codec_contex->flags,is_key_frame);
            (*_cb)(pkt.data,pkt.size,is_key_frame);
            av_free_packet(&pkt);
        }
...
bool video_encoder_ffmpeg::init(FUNC_ON_NALU *cb,T_encode_param &param)
{
    if(param.codec_id == 0)
    {
    ...
    do{
        ...
        _codec = avcodec_find_encoder((AVCodecID)param.codec_id);
        if (!_codec) {
            fprintf(stderr, "Codec not found :%d,h264:%d, h265:%d\n",param.codec_id,AV_CODEC_ID_H264,AV_CODEC_ID_HEVC);
            break;
        }
        ...
        if(_codec_contex->codec_id == AV_CODEC_ID_H264) {
            av_dict_set(&_codec_param, "preset", "slow", 0);
...
int video_encoder_ffmpeg::release()
{
    if(_codec){
