...
#include <iostream>
using std::string;
live_sdp_wrapper::live_sdp_wrapper()
{
    _eventLoopWatchVariable = 0;
}
int live_sdp_wrapper::set_stream_cb(FUNC_ON_STREAM &cb)
{
    _stream_cb = cb;
    return 0;
}
int live_sdp_wrapper::set_remote_sdp(const char *sdp)
{
...
int live_sdp_wrapper::event_loop()
{
...
int live_sdp_wrapper::start()
{
    _loop_thread = std::thread(&live_sdp_wrapper::event_loop,this);
    return 0;
}
void live_sdp_wrapper::stop()
{
...
int live_sdp_wrapper::change_sdp_listen_port(std::string &sdp, int video_port, int audio_port)
{
    int v_port = -1;
    int a_port = -1;
    int pos = sdp.find("m=");
